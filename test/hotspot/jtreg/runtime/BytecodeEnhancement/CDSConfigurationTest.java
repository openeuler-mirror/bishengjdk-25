...
/*
 * @test
 * @summary Test BytecodeEnhancement CDS dump, archived heap, and AOT boundaries.
...
public class CDSConfigurationTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        testAOTLinkedArchiveIsRejected(hashMapEnhancement);
        testLeydenModes(hashMapEnhancement);
    ...
    private static void testStaticDump(Path enhancementDir) throws Exception {
        ...
        OutputAnalyzer dump = java(
                ...
                "-XX:+AOTClassLinking",
                "-Xlog:cds=info,aot=info,class+load+enhancement=info");
        dump.shouldHaveExitValue(0)
            .shouldContain("Bytecode enhancement replacing class java/util/HashMap")
            .shouldContain("Archived java heap is not supported: Programmatically disabled")
            .shouldContain("Full module graph = disabled")
            .shouldMatch("(?s).*instance classes\\s*=\\s*\\d+, aot-linked\\s*=\\s*0,.*")
    ...
    private static void testAOTLinkedArchiveIsRejected(Path enhancementDir) throws Exception {
        Path archive = ROOT.resolve("bytecode-enhancement-aot-linked.jsa");
        java("-Xshare:dump",
             "-XX:SharedArchiveFile=" + archive,
             "-XX:+AOTClassLinking")
                .shouldHaveExitValue(0);
        ...
        java("-Xshare:on",
             ...
             "-version")
                ...
                .shouldContain("has aot-linked classes")
                .shouldContain("archived full module graph is not used");
    ...
    private static void testLeydenModes(Path enhancementDir) throws Exception {
        java("-XX:AOTMode=off",
             "-XX:BytecodeEnhancementPaths=" + enhancementDir,
             "-Xlog:class+load+enhancement=info",
             CDSConfigurationTest.class.getName(), "probe", "Updated initial capacity:")
        ...
        Path cache = ROOT.resolve("bytecode-enhancement.aot");
        java("-XX:AOTMode=record",
             "-XX:AOTCacheOutput=" + cache,
             "-XX:BytecodeEnhancementPaths=" + enhancementDir,
             "-version")
                .shouldNotHaveExitValue(0)
                .shouldContain("BytecodeEnhancement is not supported while creating or using a Leyden AOT cache");
    }

...
int SSL_UTILS_EVP_PKEY_set_alias_type(EVP_PKEY *pkey, int type)
{
    // EVP_PKEY_set_alias_type is removed from openssl 3.
    if (get_sslVersion() == V3) {
        // https://github.com/openssl/openssl/issues/14379
        KAE_TRACE("OpenSSL hint: EVP_PKEY_set_alias_type is deprecated on openssl 3.");
        return 1;
    }
...
int SSL_UTILS_EVP_PKEY_assign_EC_KEY(EVP_PKEY *pkey, void *key)
{
    ...
    return SSL_UTILS_EVP_PKEY_assign((pkey), EVP_PKEY_EC, (char *)(key));
}
...
void check_openSSL_Engine(JNIEnv *env, jboolean *init_result, char *lib_name)
{
    ...
    if (openssl_engines_path != NULL) {
        if (0 == strncmp("libssl.so.1.1", lib_name, strlen(lib_name)) && 0 != strncmp(openssl_engines_path, OPENSSL_ENGINES_VERSION_1_1, strlen(OPENSSL_ENGINES_VERSION_1_1))) {
            *init_result = JNI_FALSE;
            KAE_ThrowExceptionInInitializerError(env, "The version of OPENSSL_ENGINES in the environment is inconsistent with the version of the loaded OpenSSL library(libssl.so.1.1). Please check jdk config kae.useOpensslVersion or OPENSSL_ENGINES");
            return;
        }
        if (0 == strncmp("libssl.so.3", lib_name, strlen(lib_name)) && 0 != strncmp(openssl_engines_path, OPENSSL_ENGINES_VERSION_3_X, strlen(OPENSSL_ENGINES_VERSION_3_X))) {
            *init_result = JNI_FALSE;
            KAE_ThrowExceptionInInitializerError(env, "The version of OPENSSL_ENGINES in the environment is inconsistent with the version of the loaded OpenSSL library(libssl.so.3). Please check jdk config kae.useOpensslVersion or OPENSSL_ENGINES");
            return;
        }
    }
}

...
CompileTaskInfo::CompileTaskInfo(Method* method, int osr_bci, int comp_level, int comp_reason, int hot_cnt):
        _method(method), _osr_bci(osr_bci), _comp_level(comp_level), _comp_reason(comp_reason), _hot_count(hot_cnt) {
...
void JBoltManager::log_stacktrace(const JfrStackTrace& stacktrace) {
  ...
  for (u4 i = 0; i < framesCount; ++i) {
    const JfrStackFrame& frame = frames->at(i);
...
void JBoltManager::construct_stacktrace(const JfrStackTrace& stacktrace) {
  ...
  while (topFrameIndex < framesCount) {
    const JfrStackFrame& frame = frames->at(topFrameIndex);
...
void JBoltManager::load_order_file_phase1(int* method_cnt, size_t* segment_size) {
  ...
  *segment_size = seg_size;
  log_trace(jbolt)("Read order file method_cnt=%d, estimated_segment_size=" UINT64_FORMAT ".", mth_cnt, seg_size);
}
...
void JBoltManager::init_load_mode_phase1() {
  ...
  if (auto_mode()) {
    ...
    log_info(jbolt)("Default set JBoltCodeHeapSize=" UINT64_FORMAT " B (" UINT64_FORMAT " MB).", JBoltCodeHeapSize, JBoltCodeHeapSize / 1024 / 1024);
    return;
  }
  ...
  if (FLAG_IS_DEFAULT(JBoltCodeHeapSize)) {
    ...
    log_info(jbolt)("Auto set JBoltCodeHeapSize=" UINT64_FORMAT " B (" UINT64_FORMAT " MB).", JBoltCodeHeapSize, JBoltCodeHeapSize / 1024 / 1024);
  }
}
...
static CompileTaskInfo* create_compile_task_info(methodHandle& method) {
    ...
    CompileTaskInfo* ret = new CompileTaskInfo(method(), osr_bci, comp_level, (int)comp_reason, 0);
    return ret;
}
...
void JBoltManager::init_code_heaps(size_t non_nmethod_size, size_t profiled_size, size_t non_profiled_size, size_t cache_size, size_t ps, size_t alignment) {
  ...
  if (non_profiled_size <= jbolt_total_size) {
    vm_exit_during_initialization(err_msg("Not enough space in non-profiled code heap to split out JBolt heap(s): " UINT64_FORMAT "K <= " UINT64_FORMAT "K", non_profiled_size/K, jbolt_total_size/K));
  }
  ...
  //      Profiled nmethods
  // ---------- low ------------
  ReservedSpace rs = CodeCache::reserve_heap_memory(cache_size, ps);
  ...
  if (no_space != nullptr) {
    vm_exit_during_initialization(FormatBuffer<1024>(
        ...
        "Expect: cache_size=" UINT64_FORMAT "K, profiled_size=" UINT64_FORMAT "K, non_nmethod_size=" UINT64_FORMAT "K, jbolt_hot_size=" UINT64_FORMAT "K, non_profiled_size=" UINT64_FORMAT "K, jbolt_tmp_size=" UINT64_FORMAT "K\n"
        "Actual: cache_size=" UINT64_FORMAT "K, profiled_size=" UINT64_FORMAT "K, non_nmethod_size=" UINT64_FORMAT "K, jbolt_hot_size=" UINT64_FORMAT "K, non_profiled_size=" UINT64_FORMAT "K, jbolt_tmp_size=" UINT64_FORMAT "K\n"
        "alignment=" UINT64_FORMAT,
        no_space,
...
void JBoltManager::clear_secondary_hot_seg(TRAPS) {
...

  {
    ...
    for (CodeBlob* cb = (CodeBlob*) sec_hot->first(); cb != nullptr; cb = (CodeBlob*) sec_hot->next(cb)) {
      ...
      Method* m = nm->method();
      if (nm && nm->get_state() == nm->is_in_use() && m != nullptr) {
        mdm.add(m);
      }
    }
  }
  ...
  for (int i = 0; i < to_recompile.length(); ++i) {
    ...
    bool recompile_result = enqueue_recompile_task(cti, method, THREAD);
    if(recompile_result) {
...
CodeBlobType JBoltManager::calc_code_blob_type(Method* method, CompileTask* task, TRAPS) {
  ...
  if (method_value->get_comp_info() == nullptr) {
    CompileTaskInfo* cti = new CompileTaskInfo(method, task->osr_bci(), task->comp_level(), (int) task->compile_reason(), task->hot_count());
    if (method_value->set_comp_info(cti)) {
 ...
 */
CompileTask* JBoltManager::create_a_task_instance(CompileTaskInfo* cti, methodHandle& method, TRAPS) {
  ...
  task->initialize(compile_id, method, osr_bci, comp_level,
                  hot_count, comp_reason, is_blocking);
  return task;
}
 ...
 */
bool JBoltManager::enqueue_recompile_task(CompileTaskInfo* cti, methodHandle& method, TRAPS) {
 ...
 */
bool JBoltManager::recompile_one(CompileTaskInfo* cti, methodHandle& method, TRAPS) {
  ResourceMark rm(THREAD);
  ...
  enqueue_recompile_task(cti, method, CHECK_AND_CLEAR_false);
...
void JBoltManager::reorder_all_methods(TRAPS) {
  ...
  for (int i = 0; i < _hot_methods_sorted->length(); ++i) {
    ...
    recompile_one(cti, method, THREAD);
    if (HAS_PENDING_EXCEPTION) {

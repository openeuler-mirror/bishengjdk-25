...
public:
  CompileTaskInfo(Method* method, int osr_bci, int comp_level, int comp_reason, int hot_cnt);
  ~CompileTaskInfo();
...
private:
  ...
  static CompileTask* create_a_task_instance(CompileTaskInfo* cti, methodHandle& method, TRAPS);
  static void check_compiled_result(Method* method, CodeBlobType check_blob_type, TRAPS);
  static bool enqueue_recompile_task(CompileTaskInfo* cti, methodHandle& method, TRAPS);
  static bool recompile_one(CompileTaskInfo* cti, methodHandle& method, TRAPS);

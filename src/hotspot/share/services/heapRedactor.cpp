...
HeapRedactor::HeapRedactor(const char *redact_params_string, outputStream* out) {
  init_fields();
  if (redact_params_string != nullptr && strlen(redact_params_string) > 0) {
...
HeapRedactor::~HeapRedactor() {
#ifdef LINUX
  if(_redact_name_table != nullptr) {
    os::Linux::heap_dict_free(_redact_name_table,false);
    _redact_name_table = nullptr;
  }

  if(_redact_rules_table != nullptr) {
    os::Linux::heap_dict_free(_redact_rules_table, true);
    _redact_rules_table = nullptr;
  }

  if(_annotation_value_tables != nullptr) {
    for(uint i = 0; i < _container_nums; i++) {
      if(_annotation_value_tables[i] != nullptr) {
        os::Linux::heap_dict_free(_annotation_value_tables[i], false);
        _annotation_value_tables[i] = nullptr;
      }
    }
  }

  if(_redact_class_field_table != nullptr) {
    os::Linux::heap_dict_free(_redact_class_field_table, true);
    _redact_class_field_table = nullptr;
  }

  if(_redact_records!= nullptr) {
    for(uint i = 0; i < _container_nums; i++) {
      if(_redact_records[i] != nullptr) {
        os::Linux::heap_vector_free(_redact_records[i]);
        _redact_records[i] = nullptr;
      }
    }
  }
#endif
  ...
  if(_name_map_list != nullptr){
    FREE_C_HEAP_ARRAY(char, _name_map_list);
  }
  if(_file_name_map_list != nullptr){
    FREE_C_HEAP_ARRAY(char, _file_name_map_list);
  }
  if(_annotation_class_path != nullptr) {
    FREE_C_HEAP_ARRAY(char, _annotation_class_path);
  }
  _file_name_map_list = nullptr;
  _name_map_list = nullptr;
  _annotation_class_path = nullptr;
}
...
void HeapRedactor::init_fields() {
  ...
  _redact_name_table = nullptr;
  _redact_rules_table= nullptr;
  _annotation_value_tables = nullptr;
  _redact_records = nullptr;
  _redact_class_field_table = nullptr;
  _file_name_map_list = nullptr;
  _name_map_list = nullptr;
  _redact_class_full_name = nullptr;
  _annotation_class_path = nullptr;

  _redact_params.params_string = nullptr;
  _redact_params.heap_dump_redact = nullptr;
  _redact_params.redact_map = nullptr;
  _redact_params.redact_map_file = nullptr;
  _redact_params.annotation_class_path = nullptr;
  _redact_params.redact_password = nullptr;
}
...
char* HeapRedactor::parse_redact_child_param(char *redact_params_sub_string, const char* redact_param_prefix,
    ...
    char* pos = nullptr;
    if (redact_params_sub_string == nullptr) {
        pos = nullptr;
    } else {
...
void HeapRedactor::init(outputStream* out) {
   ...
   * if HeapDumpRedact is nullptr , jmap operation can not open redact feature without password
   * if HeapDumpRedact is not nullptr, jmap operation can not change redact level without password
   **/
    char* split_char = nullptr;
    if(RedactPassword == nullptr || (split_char = strstr(const_cast<char*>(RedactPassword), ",")) == nullptr || strlen(split_char) < SALT_LEN) {
    ...
    if(VerifyRedactPassword && !_use_sys_params) {
      ...
      if(_redact_params.redact_password == nullptr ||
...
void HeapRedactor::init_redact_map() {
  const char* map_param = nullptr;
  const char* map_file_param = nullptr;
  if (_use_sys_params) {
    map_param = RedactMap;
  ...
  if (map_file_param != nullptr) {
    read_redact_map_from_file(map_file_param);
  }
  if (map_param != nullptr) {
...
void HeapRedactor::parse_token(char* token) {
    ...
    void* _redact_rules_sub_table = _redact_class_full_name == nullptr ? nullptr :
                                    os::Linux::heap_dict_lookup(_redact_class_full_name, _redact_rules_table, false);
    if (i < token_length - 1 && _redact_rules_sub_table != nullptr) {
        token[i] = '\0';
        ...
        if (_redact_rules_sub_table == nullptr) {
            _redact_rules_sub_table = os::Linux::heap_dict_add(token, nullptr, _redact_rules_sub_table, 0);
...
HeapDumpRedactLevel HeapRedactor::init_heapdump_redact_level() {
  ...
  if (_redact_params.params_string != nullptr) {
...
bool HeapRedactor::lookup_annotation_index_in_constant_pool(AnnotationArray* field_annotations, ConstantPool *cp, int &byte_i_ref) {
    ...
    for (int calc_num_annotations = 0; calc_num_annotations < num_annotations; calc_num_annotations++) {
        ...
        if (has_anonymous_annotation) {
            ...
            if(element_name == nullptr || strcmp(element_name, "value")) {

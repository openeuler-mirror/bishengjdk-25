...
public:
  DumpWriter(const char* path, bool overwrite, AbstractCompressor* compressor, HeapRedactor* redactor = nullptr);
  ~DumpWriter();
...
DumpWriter::DumpWriter(const char* path, bool overwrite, AbstractCompressor* compressor, HeapRedactor* redactor) :
  AbstractDumpWriter(),
...
typedef char* (*CALL_DO_LOOKUP_REPLACE_VALUE)(AbstractDumpWriter*, typeArrayOop, uint);
...
class DumperSupport : AllStatic {
 public:
  ...
  static char* do_lookup_replace_value_with_symbol(AbstractDumpWriter* writer, typeArrayOop array, uint container_id);
  static char* do_lookup_replace_value_with_char(AbstractDumpWriter* writer, typeArrayOop array, uint container_id);
  ...
  // HeapVector Records
  static void do_heapVector(AbstractDumpWriter* writer);
...
char* DumperSupport::do_lookup_replace_value_with_symbol(AbstractDumpWriter* writer, typeArrayOop array, uint container_id) {
    address obj_addr = cast_from_oop<address>(array);
    Symbol* anonymous_value_symbol = writer->heapRedactor()->lookup_replace_value<Symbol*>(obj_addr, container_id);
...
char* DumperSupport::do_lookup_replace_value_with_char(AbstractDumpWriter* writer, typeArrayOop array, uint container_id) {
    address obj_addr = cast_from_oop<address>(array);
    char* anonymous_value = writer->heapRedactor()->lookup_replace_value<char*>(obj_addr, container_id);
...
bool DumperSupport::dump_replace_value(CALL_DO_LOOKUP_REPLACE_VALUE fn, AbstractDumpWriter* writer, typeArrayOop array, uint container_id) {
    ...
    anonymous_value = fn(writer, array, container_id);
...
CALL_DUMP_INSTANCE_FIELDS_DESCRIPTORS ClassDumper::_dump_instance_fields_descriptors = nullptr;
...
class HeapObjectDumper : public ObjectClosure {
 ...
 public:
  HeapObjectDumper(AbstractDumpWriter* writer, UnmountedVThreadDumper* vthread_dumper, CALL_DUMP_PRIM_ARRAY fn = DumperSupport::dump_prim_array)
    : _writer(writer), _vthread_dumper(vthread_dumper), _redact_dump_prim_array(fn) {}
...
class HeapObjectRedactDumper : public ObjectClosure {
  ...
  public:
    HeapObjectRedactDumper(AbstractDumpWriter* writer, UnmountedVThreadDumper* vthread_dumper, CALL_DO_LOOKUP_REPLACE_VALUE  do_lookup_replace_value, uint id = 0)
      : _writer(writer), _vthread_dumper(vthread_dumper), _do_lookup_replace_value(do_lookup_replace_value), container_id(id) {}
...
void DumpMerger::merge_done() {
  ...
  if (!_has_error) {
    DumperSupport::do_heapVector(_writer);
...
void DumperSupport::do_heapVector(AbstractDumpWriter* writer) {
  ...
  const uint container_nums = writer->heapRedactor()->get_container_nums();
  for (uint container_index = 0; container_index < container_nums; container_index++) {
    ...
    while (vector_node != nullptr && items != nullptr) {
      for (i = 0; i < node_len; i++) {
        typeArrayOop array = (typeArrayOopDesc*)items[i];
        char *anonymous_value = nullptr;
        for (uint c_index = 0; c_index < container_nums; c_index++) {
          anonymous_value = fn(writer, array, c_index);
          if(anonymous_value != nullptr) {
              break;
          }
        }
        int length = anonymous_value == nullptr ? array->length() : strlen(anonymous_value);
...
void VM_HeapDumper::work(uint worker_id) {
  ...
  if (is_vm_dumper(dumper_id)) {
    ...
    // HPROF_UTF8 records
    if(writer()->heapRedactor() != nullptr && (writer()->heapRedactor()->redact_level() == REDACT_NAMES ||
  ...
  DumpWriter segment_writer(DumpMerger::get_writer_path(writer()->get_file_path(), dumper_id),
                            writer()->is_overwrite(), writer()->compressor(), writer()->heapRedactor());
  if (!segment_writer.has_error()) {
    ...
    if (!is_parallel_dump()) {
      if(writer()->heapRedactor() != nullptr && (writer()->heapRedactor()->redact_level() == REDACT_BASIC ||
                                              writer()->heapRedactor()->redact_level() == REDACT_FULL)) {
        HeapObjectDumper obj_dumper(&segment_writer, this, DumperSupport::redact_basic_dump_prim_array);
        Universe::heap()->object_iterate(&obj_dumper);
      } else if(writer()->heapRedactor() != nullptr && writer()->heapRedactor()->redact_level() == REDACT_ANNOTATION) {
        HeapObjectRedactDumper obj_dumper(&segment_writer, this, DumperSupport::do_lookup_replace_value_with_symbol);
        Universe::heap()->object_iterate(&obj_dumper);
      } else if(writer()->heapRedactor() != nullptr && writer()->heapRedactor()->redact_level() == REDACT_DIYRULES) {
        HeapObjectRedactDumper obj_dumper(&segment_writer, this, DumperSupport::do_lookup_replace_value_with_char);
        Universe::heap()->object_iterate(&obj_dumper);
      } else {
      ...
      if(writer()->heapRedactor() != nullptr && (writer()->heapRedactor()->redact_level() == REDACT_BASIC ||
         ...
         HeapObjectRedactDumper obj_dumper(&segment_writer, this, DumperSupport::do_lookup_replace_value_with_symbol, worker_id);
         _poi->object_iterate(&obj_dumper, worker_id);
      } else if(writer()->heapRedactor() != nullptr && writer()->heapRedactor()->redact_level() == REDACT_DIYRULES) {
         HeapObjectRedactDumper obj_dumper(&segment_writer, this, DumperSupport::do_lookup_replace_value_with_char, worker_id);
         _poi->object_iterate(&obj_dumper, worker_id);
      } else {
        HeapObjectDumper obj_dumper(&segment_writer, this);
        _poi->object_iterate(&obj_dumper, worker_id);
...
int HeapDumper::dump(const char* path, outputStream* out, int compression, bool overwrite, uint num_dump_threads) {
  ...
  DumpWriter writer(_path, overwrite, compressor);
  ...
  DumpMerger merger(_path, &writer, dumper.dump_seq());

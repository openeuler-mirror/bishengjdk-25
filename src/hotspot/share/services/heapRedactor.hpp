...
public:
    ...
    template<typename T>
    T lookup_replace_value(void* key, uint container_id) {
        void* val = nullptr;
#ifdef LINUX
        if(_annotation_value_tables[container_id] == nullptr) {
            return nullptr;
        }
        val = os::Linux::heap_dict_lookup(key, _annotation_value_tables[container_id], true);
#endif
        if(val != nullptr) {
            return (T)val;
        }
